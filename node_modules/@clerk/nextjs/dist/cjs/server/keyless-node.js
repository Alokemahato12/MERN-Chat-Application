...
function updateGitignore() {
  ...
  const { existsSync, writeFileSync, readFileSync, appendFileSync } = import_safe_node_apis.default.fs;
  ...
  const gitignorePath = import_safe_node_apis.default.path.join(process.cwd(), ".gitignore");
...
const generatePath = (...slugs) => {
  ...
  return import_safe_node_apis.default.path.join(process.cwd(), CLERK_HIDDEN, ...slugs);
};
...
function safeParseClerkFile() {
  ...
  const { readFileSync } = import_safe_node_apis.default.fs;
  try {
...
async function createOrReadKeyless() {
  ...
  const { existsSync, writeFileSync, mkdirSync, rmSync } = import_safe_node_apis.default.fs;
  ...
  if ((envVarsMap == null ? void 0 : envVarsMap.publishableKey) && (envVarsMap == null ? void 0 : envVarsMap.secretKey)) {
    ...
    import_logger.logger.logOnce(createMessage(envVarsMap));
    return envVarsMap;
  }
  ...
  import_logger.logger.logOnce(createMessage(accountlessApplication));
